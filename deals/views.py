        ...
        messages=[
            {"role": "system", "content": "Ты менеджер по логистике со стажем в 5 лет и опытом в различных вариантах клиентских запросов"},
            {
                ...
                "content": f"Извлеки данные о грузе и верни ответ в формате чистый json("
                           ...
                           f"load_place и unload_place не дели по городу и адресу а запиши в одно поле)."
                           f"Есть еще несколько моментов,которые важно учитывать:"
                           f"Например запрос может содержать географические параметра(место погрузки "
                           f"и место выгрузки в таком формате(к примеру): Луга,Петропавлоск, где "
                           f"Луга это место погрузки а Петропавлоск место выгрузки. Также всегда проверяй слова на "
                           f"их соотвествия сленговым названием населенных пунктов, к примеру мск=москва, питер-петербург, "
                           f"новосиб-новосибирск, новорос-новороссийск(ищи по полному списку в интернете) и если "
                           f"подобное совпадение найдется то возвращай всегда полное название-к примеру найдешь питер а вернешь Санкт-Петербург."
                           f" Так что если ты в запросе найдешь"
                           f"2 и больше городов-всегда используй первый как город погрузки а все остальные как города выгрузки."
                           f" Если нет четкого указания на наименование груза, всегда сравнивай каждое слово из запроса на соответствие"
                           f"  товару. Если к примеру одно слово соответствует и товару и населенному пункту, но в тексте уже найдено 2 "
                           f"населенных пункта или указано на конкретные города погрузки и выгрузки то используй это слово как товар."
                           f"Населенные пункты через запятую в формате город1,город2 означают что город1 это место погрузки а"
                           f" город2 это место выгрузки, единицы измерения, такие как тонны,метры и так далее не могут быть обозначениями груза."
                           f" Некоторые наши клиенты работают с иностранными"
                           f" поставщиками и наименования груза пишут латиницей,"
                           f"так же как и единицы измерения, к примеру kg,kgs-килограммы и так далее,"
                           f"так что если в запросе по твоему мнению чего то не хватает и есть непонятный текст латиницей,"
                           f"попробуй прочесть его транслитом."
                           f" Также пожалуйста указывай единицы измерений где применимо:{data}"
            }
        ],
        temperature = 0.2
    )
